...
const EditExamModal = ({
  ...
  const { updateExam, loading, error } = useExam();
  ...
  useEffect(() => {
    if (exam) {
      setFormData({
        ...
        exam_duration: exam.exam_duration || 120,
        exam_type: exam.exam_type || "TOEIC",
        certificate_id: exam.certificate_id?.toString() || "",
        source: exam.source || "",
      });
    }
  }, [exam]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await updateExam(exam.exam_id, examData);
    if (success) {
      onSuccess();
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-bold text-gray-800">Chỉnh sửa đề thi</h2>
          <button
            onClick={onClose}
