...
export const analyzeExamPerformance = async (user_exam_id, user_id) => {
  ...
  // 5. Fetch roadmaps & courses từ DB để AI đề xuất cụ thể
  const Op = db.Sequelize.Op;

  let suggestedRoadmaps = await db.Roadmap.findAll({
    ...
    include: [
      {
        ...
        where: {
          certificate_name: { [Op.like]: `%${examInfo.exam_type}%` },
        },
        required: true,
      },
    ],
    ...
    order: [["roadmap_level", "ASC"]],
  });

  // Fallback nếu không khớp exam_type
  if (suggestedRoadmaps.length === 0) {
    suggestedRoadmaps = await db.Roadmap.findAll({
      where: { roadmap_status: true },
      include: [
        {
          model: db.Certificate,
  ...
  let suggestedCourses = await db.Course.findAll({
    where: {
      course_status: true,
      tag: { [Op.like]: `%${examInfo.exam_type}%` },
    },
    attributes: [
      "course_id",
      "course_title",
    ...
    order: [["course_level", "ASC"]],
    limit: 8,
  });

  // Fallback nếu không khớp exam_type
  if (suggestedCourses.length === 0) {
    suggestedCourses = await db.Course.findAll({
  ...
  const roadmapContextStr =
    suggestedRoadmaps.length > 0
      ? suggestedRoadmaps
          .map(
            (r) =>
              `  [ID:${r.roadmap_id}] "${r.roadmap_title}" — Cấp độ: ${r.roadmap_level}, Thời lượng: ~${r.estimated_duration ?? "?"}h`,
          )
          .join("\n")
      : "  - Chưa có lộ trình trong hệ thống";
  ...
  const courseContextStr =
    suggestedCourses.length > 0
      ? suggestedCourses
          .map(
            (c) =>
              `  [ID:${c.course_id}] "${c.course_title}" — Level: ${c.course_level}, Thời lượng: ~${c.estimate_duration ?? "?"}h, Tag: ${c.tag ?? "N/A"}`,
          )
          .join("\n")
...
� LỘ TRÌNH SẴN CÓ TRONG HỆ THỐNG:
${roadmapContextStr}

📖 KHÓA HỌC SẴN CÓ TRONG HỆ THỐNG:
${courseContextStr}
...
6. Dựa vào danh sách LỘ TRÌNH và KHÓA HỌC CÓ SẴN TRONG HỆ THỐNG ở trên, đề xuất CỤ THỂ: nên bắt đầu lộ trình nào, học khóa học nào trước. Gọi đúng tên lộ trình/khóa học, giải thích ngắn gọn lý do phù hợp với điểm yếu của học viên.
...
Trả lời bằng tiếng Việt, ngắn gọn, dễ hiểu, có cấu trúc rõ ràng.`;
  ...
  return {
    ...
    data: {
      ...
      suggested_roadmaps: suggestedRoadmaps.map((r) => ({
        roadmap_id: r.roadmap_id,
        roadmap_title: r.roadmap_title,
        roadmap_level: r.roadmap_level,
        estimated_duration: r.estimated_duration,
        certificate_name: r.Certificate?.certificate_name ?? null,
      })),
      suggested_courses: suggestedCourses.map((c) => ({
        course_id: c.course_id,
        course_title: c.course_title,
        course_level: c.course_level,
        estimate_duration: c.estimate_duration,
        tag: c.tag,
      })),
